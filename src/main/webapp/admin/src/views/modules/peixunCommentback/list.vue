		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '姓名' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.yonghuName" placeholder="姓名"
							clearable></el-input>
					</el-form-item>
				...
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('peixunCommentback','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">新增</el-button>
						&nbsp;
						<el-button v-if="isAuth('peixunCommentback','删除')" :disabled="dataListSelections.length <= 0"
							type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
						&nbsp;
						<el-button v-if="isAuth('peixunCommentback','报表')" type="success" icon="el-icon-pie-chart"
							@click="chartDialog()">报表</el-button>
						&nbsp;
						<a style="text-decoration:none" class="el-button el-button--success"
							v-if="isAuth('peixunCommentback','导入导出')" icon="el-icon-download"
							href="http://localhost:8080/gaoiaozhinengpeixun/upload/peixunCommentbackMuBan.xls">批量导入培训评价数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('peixunCommentback','导入导出')" style="display: inline-block"
							action="gaoiaozhinengpeixun/file/upload" :on-success="peixunCommentbackUploadSuccess"
							:on-error="peixunCommentbackUploadError" :show-file-list=false>
							<el-button v-if="isAuth('peixunCommentback','导入导出')" type="success" icon="el-icon-upload2">
								批量导入培训评价数据</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						<download-excel v-if="isAuth('peixunCommentback','导入导出')" style="display: inline-block"
							class="export-excel-wrapper" :data="dataList" :fields="json_fields"
							name="peixunCommentback.xls">
							<!-- 导出excel -->
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('peixunCommentback','查看')" :data="dataList" v-loading="dataListLoading"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="peixunCommentbackText" header-align="center" label="评价内容">
						<template slot-scope="scope">
							<span
								v-if="scope.row.peixunCommentbackText != null &&scope.row.peixunCommentbackText.length>10">
								{{scope.row.peixunCommentbackText.slice(0,10)}}...
							</span>
							<span v-else>
								{{scope.row.peixunCommentbackText}}
							</span>
						</template>
					</el-table-column>

					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="insertTime"
						header-align="center" label="评价时间">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('peixunCommentback','查看')" type="success" icon="el-icon-tickets"
								size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button
								v-if="isAuth('peixunCommentback','修改')"
								type="primary" icon="el-icon-edit" size="mini"
								@click="addOrUpdateHandler(scope.row.id)">回复</el-button>

							<el-button v-if="isAuth('peixunCommentback','删除')" type="danger" icon="el-icon-delete"
	...
	export default {
		data() {
			return {
				...
				json_fields: {
					//级联表字段
					'课程名称': 'peixunUuidNumber',
					'培训开始时间': 'peixunOnTime',
					'培训结束时间': 'peixunDownTime',
					'培训类型': 'peixunTypes',
					'成绩': 'peixunFenshu',
					'姓名': 'yonghuName',
					'手机号': 'yonghuPhone',
		...
		methods: {
			chartDialog() {
				...
				let params = {
					...
					thisTable: { //当前表
						tableName: 'peixun_commentback', //当前表表名,
						sumColum: 'peixun_commentback_number', //求和字段
						date: 'insert_time', //分组日期字段
						// string : 'peixun_commentback_name',//分组字符串字段
						// types : 'peixun_commentback_types',//分组下拉框字段
					},
					// joinTable : {//级联表（可以不存在）
				...
				//         groupColumn: "peixun_commentback_types",
				//     }
				//     this.$http({
			...
			getDataList() {
				...
				if (this.searchForm.peixunUuidNumber != '' && this.searchForm.peixunUuidNumber != undefined) {
					params['peixunUuidNumber'] = '%' + this.searchForm.peixunUuidNumber + '%'
				}
				...
				params['peixunCommentbackDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除


				this.$http({
					url: "peixunCommentback/page",
					method: "get",
					params: params
				}).then(({
					data
			...
			// 导入功能上传文件成功后调用导入方法
			peixunCommentbackUploadSuccess(data) {
				let _this = this;
				_this.$http({
					url: "peixunCommentback/batchInsert?fileName=" + data.file,
					method: "get"
				}).then(({
					data
				}) => {
			...
			// 导入功能上传文件失败后调用导入方法
			peixunCommentbackUploadError(data) {
